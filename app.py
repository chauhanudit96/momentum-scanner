...
def score(df, r6, r12, rs, regime_pen, pattern, pattern_conf):
    ...
    else:              sc["rsi"]=0;  flags.append(("bear",f"RSI {rsi_:.0f} — {'extremely overbought' if rsi_>80 else 'downtrend'}"))
...
def tile(label, val, col="#e0e8ff", sub=""):
    return f"""<div style="background:#060e1c;border:1px solid {col}25;border-radius:8px;padding:10px 12px;">
      ...
      {"" if not sub else f'<div style="color:#5c7099;font-size:10px;margin-top:2px;">{sub}</div>'}
    </div>"""
...
def main():
    ...
    for idx, (msg, col, pen) in regime_data.items():
        ...
        regime_html += f"<div style='margin-bottom:4px;'><span style='color:#5c7099;font-size:10px;font-weight:600;'>{idx}:</span> <span style='color:{col};font-weight:700;font-size:13px;'>{dot} {msg}</span>{f' <span style=\"color:#5c7099;font-size:11px;\">(-{pen} pts)</span>' if pen>0 else ''}</div>"
                ...
                if df is None:
                    ...
                    <div style="background:#0d1b2e;border:1px solid #1a2744;border-radius:10px;padding:16px;margin-bottom:12px;">
                      <div style="display:flex;justify-content:space-between;">
                        <div>
                          <div style="color:#90caf9;font-size:11px;font-weight:600;letter-spacing:1px;">{ticker} · {df.index[-1].strftime('%d %b %Y')} · Turnover ₹{to/1e7:.1f}Cr/day ✓</div>
                          <div style="color:#e0e8ff;font-size:26px;font-weight:900;margin-top:4px;">₹{p:,.2f}</div>
                        </div>
                        <div style="text-align:right;">
                          <div style="color:{chg_col};font-size:16px;font-weight:700;">{'▲' if day_chg>=0 else '▼'} {abs(day_chg):.2f}%</div>
                          {f'<div style="color:#f44336;font-size:11px;font-weight:600;">⚠️ BIG MOVE TODAY — spike penalty applied</div>' if abs(today_mv)>=5 else ''}
                        </div>
                      </div>
                    </div>
                    """, unsafe_allow_html=True)
                    ...
                    with c1: st.markdown(tile("52W HIGH",f"₹{h52:,.1f}",ph_col,"ABOVE ✓" if pct_h<=0 else f"{pct_h:.1f}% below"),unsafe_allow_html=True)
                    with c2: st.markdown(tile("VOLUME",f"{vr_:.1f}x avg","#00c853" if vr_>=1.5 else "#ff9100" if vr_>=1 else "#f44336"),unsafe_allow_html=True)
                    with c3: st.markdown(tile("RSI",f"{rsi_:.0f}","#00c853" if 50<=rsi_<=65 else "#ff9100" if rsi_<80 else "#f44336"),unsafe_allow_html=True)
                    with c4: st.markdown(tile("MACD",f"{'▲' if mac_>0 else '▼'} {mac_:.2f}","#00c853" if mac_>0 and float(l["HIST"])>0 else "#ff9100" if mac_>0 else "#f44336"),unsafe_allow_html=True)

                    c5,c6,c7,c8 = st.columns(4)
                    with c5: st.markdown(tile("EMA 20",f"₹{e20:,.1f}","#00c853" if p>e20 else "#f44336","✓ above" if p>e20 else "✗ below"),unsafe_allow_html=True)
                    with c6: st.markdown(tile("EMA 50",f"₹{e50:,.1f}","#00c853" if e20>e50 else "#f44336","✓ 20>50" if e20>e50 else "✗"),unsafe_allow_html=True)
                    with c7: st.markdown(tile("EMA 200",f"₹{e200:,.1f}","#00c853" if e50>e200 else "#f44336","✓ 50>200" if e50>e200 else "✗"),unsafe_allow_html=True)
                    r6c="#00c853" if r6 and r6>=20 else "#ff9100" if r6 and r6>=0 else "#f44336"
                    with c8: st.markdown(tile("6M RETURN",f"+{r6:.1f}%" if r6 and r6>=0 else f"{r6:.1f}%" if r6 else "—",r6c),unsafe_allow_html=True)
                    ...
                    <div style="background:#060e1c;border:2px solid {vc}50;border-radius:14px;padding:22px;text-align:center;margin-bottom:14px;">
                      ...
                      <div style="color:#5c7099;font-size:11px;margin-bottom:12px;">OUT OF 100 · Raw: {raw}{f' · Spike: {sp}' if sp<0 else ''}{f' · Regime: -{regime_pen}' if regime_pen>0 else ''}</div>
                      ...
                      {"<div style='color:#f44336;font-size:13px;font-weight:700;margin-top:12px;'>🚫 NO TRADE IS THE BEST TRADE</div>" if verdict=="AVOID" else f"<div style='color:{vc};font-size:13px;font-weight:700;margin-top:12px;'>→ {'BUY BREAKOUT' if final>=80 else 'TRADABLE — HALF SIZE'}</div>"}
                    </div>
                    """, unsafe_allow_html=True)

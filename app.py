...
def setup_badge(sname):
    ...
    st.markdown(
        ...
        "<span style='color:#6b7280;font-size:12px;'>📊 Min score: <b style='color:#d0d7e3;'>" + ("N/A" if sname == "NO_SETUP" else str(meta["min_score"]) + "/100") + "</b></span>"
        "</div></div>",
        unsafe_allow_html=True
    )
        ...
        for key, meta in SETUPS.items():
            ...
            with st.expander(meta["icon"] + "  " + meta["name"] + "  —  " + meta["tagline"]):
                ...
                with c2:
                    ...
                    st.metric("Min Score Required", "N/A" if key == "NO_SETUP" else str(meta["min_score"]) + "/100")

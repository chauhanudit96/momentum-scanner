...
def make_trade(sname, sd, raw, capital, risk_pct, regime_pen):
    ...
    if sname == "NO_SETUP" or final < meta["min_score"]:
        return {"ok":False,"final":final,"verdict":"NO TRADE","vc":"#6b7280",
                "reason":"Score " + str(final) + " is below minimum " + str(meta["min_score"]) + " required for " + meta["name"] + ". Wait for a better setup."}
    if final >= 90:   verdict, vc = "ELITE SETUP",  "#10b981"
        ...
        return {"ok":False,"final":final,"verdict":"BELOW MINIMUM","vc":"#ef4444",
                "reason":"Score " + str(final) + " is below minimum " + str(meta["min_score"]) + " for this setup type. Quality insufficient — wait."}
...
def render_result(res, regime_pen):
    ...
    with col_a:
        ...
        score_card(final, verdict, vc, "Raw " + str(res["raw"]) + "  →  " + rp_s + sp_s + "  Final: " + str(final))
        ...
        if final >= 90:
            ...
            st.error("**AVOID** — Score below minimum for this setup type. No trade today.")
    ...
    # Score breakdown
    st.markdown("#### Score Breakdown — " + meta["name"])
